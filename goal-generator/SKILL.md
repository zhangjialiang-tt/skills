...
| Proposed target | Skill 推荐的目标 | 必须携带推导依据（现有 SLO / 真实基线 / 行业或体验目标）；无依据时不给具体数字，改为先测量并提出 2-3 个候选；重大影响时需用户确认 |

```
❌ 错误：用户说"好几秒" → 验收写"P95 < 500ms（当前约 2-3s）"
✅ 正确：用户说"好几秒" → 验收写"响应时间目标：先测量当前 P50/P95/P99 基线并读取现有 SLO；若无可复用目标，基于真实基线提出 2-3 个候选供用户确认后写入"
