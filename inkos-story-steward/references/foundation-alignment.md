...
> 同时读取 `design-to-inkos-compilation.md`，按编译映射执行反向损失检查。
...
- 分卷规划被压缩（多卷结构变成笼统概述）
- 风格要求被忽略
- InkOS 文件出现无法追溯到设计源的新规则、角色关系或伏笔
...
同时核对 `10-readiness-review.md` 中的编译覆盖记录：

- `mapped` 项在 InkOS 文件中缺失或含义变化 → 对齐失败；
- `design-only` 项被扩写成无来源事实 → 对齐失败；
- InkOS 构件出现无法追溯到 00-10 的规则、关系或伏笔 → 对齐失败。
